...
/* ═══════════════════════════════════════════════════════════════════
   DỮ LIỆU TOÀN CỤC
═══════════════════════════════════════════════════════════════════ */
static Staff    gStaff[MAX_STAFF]; static int    gStaffCount = 0;
static MenuItem gMenu[MAX_MENU];   static int    gMenuCount  = 0;
...
static void DrawTxtCenter(const char *s, Rectangle r, float sz, Color c, bool bold) {
    Vector2 m = bold ? MeasureB(s,sz) : Measure(s,sz);
    float tx = r.x + (r.width  - m.x) * 0.5f;
    float ty = r.y + (r.height - m.y) * 0.5f;
    if (bold) DrawTextEx(gFontB, s, (Vector2){tx,ty}, sz, 1.f, c);
    else      DrawTextEx(gFont,  s, (Vector2){tx,ty}, sz, 1.f, c);
}
static void DrawTxtVCL(const char *s, Rectangle r, float sz, Color c, float padL, bool bold) {
    Vector2 m = bold ? MeasureB(s,sz) : Measure(s,sz);
    float ty = r.y + (r.height - m.y) * 0.5f;
    if (bold) DrawTextEx(gFontB, s, (Vector2){r.x+padL, ty}, sz, 1.f, c);
    else      DrawTextEx(gFont,  s, (Vector2){r.x+padL, ty}, sz, 1.f, c);
}
...
static void normalizeVN(const char *src, char *dst) {
    ...
    char tmp[2048]; strncpy(tmp, src, 2047); tmp[2047] = 0;
    for (int k = 0; k < 134; k++) {
        char *p;
    ...
    int i = 0;
    for (; tmp[i]; i++) dst[i] = (char)tolower((unsigned char)tmp[i]);
    dst[i] = 0;
}
...
static int levenshtein(const char *a, const char *b) {
    ...
    if (!col) return 99;
    for (int y=0;y<=la;y++) col[y]=y;
    for (int x=1;x<=lb;x++) {
        col[0]=x;
        for (int y=1,ld=x-1;y<=la;y++){int od=col[y];col[y]=MIN3(col[y]+1,col[y-1]+1,ld+(a[y-1]==b[x-1]?0:1));ld=od;}
    }
    int r=col[la]; free(col); return r;
}

static bool matchSearch(const char *query, const char *target) {
    if (!query[0]) return true;
    char nq[2048], nt[2048];
    normalizeVN(query, nq); normalizeVN(target, nt);
...
static void saveStaff(void) {
    FILE *fp = fopen("staff.txt","w"); if (!fp) return;
    fprintf(fp,"%d\n",gStaffCount);
    for (int i=0;i<gStaffCount;i++)
        fprintf(fp,"%d\n%s\n%s\n%s\n%s\n%.2f\n%.2f\n%d\n%ld\n",
            gStaff[i].id, gStaff[i].username, gStaff[i].password,
            gStaff[i].name, gStaff[i].position,
            gStaff[i].salaryPerHour, gStaff[i].totalSalaryEarned,
            gStaff[i].status, gStaff[i].lastCheckIn);
    fclose(fp);
}
static void loadStaff(void) {
    FILE *fp = fopen("staff.txt","r");
    if (fp) {
        if (fscanf(fp,"%d\n",&gStaffCount)!=1){fclose(fp);return;}
        for (int i=0;i<gStaffCount;i++){
            fscanf(fp,"%d\n",&gStaff[i].id);
            fgets(gStaff[i].username,SHORT_STR,fp); gStaff[i].username[strcspn(gStaff[i].username,"\n")]=0;
            fgets(gStaff[i].password,SHORT_STR,fp); gStaff[i].password[strcspn(gStaff[i].password,"\n")]=0;
            fgets(gStaff[i].name,    STR_LEN,  fp); gStaff[i].name[strcspn(gStaff[i].name,"\n")]=0;
            fgets(gStaff[i].position,SHORT_STR,fp); gStaff[i].position[strcspn(gStaff[i].position,"\n")]=0;
            long ci;
            fscanf(fp,"%f\n%f\n%d\n%ld\n",
                &gStaff[i].salaryPerHour,&gStaff[i].totalSalaryEarned,
                &gStaff[i].status,&ci);
        ...
        gStaffCount=1; saveStaff();
    }
}
static void saveMenu(void) {
    FILE *fp = fopen("menu.txt","w"); if (!fp) return;
    fprintf(fp,"%d\n",gMenuCount);
    for (int i=0;i<gMenuCount;i++)
        fprintf(fp,"%d\n%s\n%.0f\n%d\n",gMenu[i].id,gMenu[i].name,gMenu[i].price,gMenu[i].inStock);
    fclose(fp);
}
static void loadMenu(void) {
    FILE *fp = fopen("menu.txt","r");
    if (fp) {
        if (fscanf(fp,"%d\n",&gMenuCount)!=1){fclose(fp);return;}
        for (int i=0;i<gMenuCount;i++){
            fscanf(fp,"%d\n",&gMenu[i].id);
            fgets(gMenu[i].name,STR_LEN,fp); gMenu[i].name[strcspn(gMenu[i].name,"\n")]=0;
        ...
        gMenuCount=4; saveMenu();
    }
}
static void saveInvoice(int t) {
    time_t now=time(NULL); struct tm *ti=localtime(&now);
    char fn[64]; sprintf(fn,"hoadon_%02d_%02d_%04d.txt",ti->tm_mday,ti->tm_mon+1,ti->tm_year+1900);
    FILE *fp=fopen(fn,"a");
    if (fp){
        fprintf(fp,"================================================================\n");
...
static Font loadVietFont(const char *path, int size) {
    int cp[700]; int n=0;
    /* ASCII */
    for (int i=32;i<127;i++) cp[n++]=i;
    /* Latin Extended */
    int ext[]={0xC0,0xC1,0xC2,0xC3,0xC8,0xC9,0xCA,0xCC,0xCD,
               0xD2,0xD3,0xD4,0xD5,0xD9,0xDA,0xDD,
               0xE0,0xE1,0xE2,0xE3,0xE8,0xE9,0xEA,0xEC,0xED,
               0xF2,0xF3,0xF4,0xF5,0xF9,0xFA,0xFD,
               0x102,0x103,0x110,0x111,0x1A0,0x1A1,0x1AF,0x1B0};
    for (int i=0;i<40;i++) cp[n++]=ext[i];
    /* Latin Extended Additional (toàn bộ tiếng Việt) */
    for (int i=0x1EA0;i<=0x1EF9;i++) cp[n++]=i;
    /* Ký hiệu tiền VND + các dấu */
    int sym[]={0x20AB,0x2014,0x2013,0x2022,0x25CF,0x2713,0x2715,0x00B7,0x00B0};
    for (int i=0;i<9;i++) cp[n++]=sym[i];
...
static Font tryLoadFont(const char *paths[], int size) {
    for (int i=0; paths[i]; i++) {
        if (FileExists(paths[i])) return loadVietFont(paths[i], size);
    }
...
static void resetScreen(Screen s) {
    ...
    gSearch[0] = 0; gSearchLen = 0;
}

static void showToast(const char *msg, Color col) {
    strncpy(gToastMsg, msg, 255);
    gToastTimer = 3.0f; gToastColor = col; gToastActive = true;
}
...
static bool Button(Rectangle r, const char *label, Color bg, Color bgHov, int slot) {
    ...
    Color dc = {
        (unsigned char)lerp((float)bg.r,(float)bgHov.r,t),
        (unsigned char)lerp((float)bg.g,(float)bgHov.g,t),
        (unsigned char)lerp((float)bg.b,(float)bgHov.b,t),
        255
    };
    /* bóng đổ */
    DrawRectangleRounded((Rectangle){r.x+2,r.y+3,r.width,r.height},0.3f,8,
        (Color){120,160,255,(unsigned char)(20*t)});
    DrawRectangleRounded(r, 0.45f, 8, dc);
    /* viền sáng khi hover */
    if (t > 0.02f) {
        Color bc = {CA_GOLD_LITE.r, CA_GOLD_LITE.g, CA_GOLD_LITE.b, (unsigned char)(140*t)};
        DrawRectangleRoundedLines(r, 0.45f, 8, bc);
    }
    DrawTxtCenter(label, r, 14.f, CT_WHITE, false);
...
static bool SmallBtn(Rectangle r, const char *label, Color bg, Color bgH, int slot) {
    ...
    DrawTxtCenter(label, r, 12.f, CT_WHITE, false);
    if (hov) SetMouseCursor(MOUSE_CURSOR_POINTING_HAND);
    return clk;
}
...
/* ═══════════════════════════════════════════════════════════════════
   Ô NHẬP LIỆU
═══════════════════════════════════════════════════════════════════ */
static void InputField(Rectangle r, int idx, const char *placeholder, float fs) {
    Vector2 mp = GetMousePosition();
    bool hov = CheckCollisionPointRec(mp, r);
    bool active = (gActiveInp == idx);
    if (hov && IsMouseButtonPressed(MOUSE_LEFT_BUTTON)) {
        gActiveInp = idx; active = true;
    }
    ...
    if (gInpPass[idx]) {
        ...
        for (const char *p = gInp[idx]; *p;) {
            if ((*p & 0xC0) != 0x80) { masked[nc++] = '*'; }
            p++;
        }
        masked[nc] = 0; display = masked;
    }
...
static void processInput(void) {
    ...
    while (cp > 0) {
        char buf[5] = {0}; int nb = utf8Encode(cp, buf);
        if (nb > 0 && gInpLen[idx] + nb < 1020) {
            for (int i=0;i<nb;i++) gInp[idx][gInpLen[idx]++] = buf[i];
...
static void drawSidebar(void) {
    ...
    float cx = sw * 0.5f, cy = 38.f;
    DrawCircle((int)cx, (int)cy, 22, (Color){90,140,255,255});
    DrawCircle((int)cx, (int)cy, 16, CA_GOLD);
    DrawCircle((int)cx, (int)cy, 10, (Color){90,140,255,255});
    DrawCircle((int)cx, (int)cy,  5, CA_GOLD);
    /* steam */
    for (int i=0;i<3;i++) {
        float ox = (i-1)*8.f;
    ...
    int cardY = 110;
    if (gCurrentStaff) {
        DrawRectangle(8, cardY, sw-16, 54, CB_PANEL);
        DrawRectangleLines(8, cardY, sw-16, 54, CB_BORDER);
        /* tên */
        Vector2 nv = MeasureB(gCurrentStaff->name, 13);
        float nx = fmaxf(10.f, (sw - nv.x) * 0.5f);
        DrawTextEx(gFontB, gCurrentStaff->name, (Vector2){nx, cardY+8.f}, 13.f, 1.f, CT_WHITE);
        /* chức vụ */
        Vector2 pv = Measure(gCurrentStaff->position, 11);
        DrawTxtL(gCurrentStaff->position, (sw-pv.x)*.5f, cardY+28.f, 11.f, CA_GOLD);
    }

    /* ── Menu điều hướng ── */
    typedef struct { const char *label; Screen scr; bool logout; } NavItem;
    NavItem nav[] = {
        {"  Sơ đồ Bàn",   SCR_TABLES,  false},
        {"  Thực đơn",     SCR_MENU,    false},
        {"  Nhân viên",    SCR_STAFF,   false},
        {"  Thống kê",     SCR_STATS,   false},
        {"  Biểu đồ",      SCR_CHART,   false},
        {"",               -1,          false},   /* separator */
        {"  Đăng xuất",    -1,          true },
    };
    int startY = gCurrentStaff ? 172 : 112;
    int bH = 38, bPad = 6;

    for (int i = 0; i < 7; i++) {
        if (!nav[i].label[0]) continue;  /* separator */
        Rectangle r = {(float)bPad, (float)(startY + i*(bH+4)), (float)(sw-bPad*2), (float)bH};
        Vector2 mp = GetMousePosition();
        bool hov = CheckCollisionPointRec(mp, r);
        bool isAct = !nav[i].logout && gScreen == nav[i].scr;

        Color bg = isAct ? (Color){120,170,255,255} : (hov ? (Color){210,225,255,255} : CB_SIDEBAR);
        ...
        Color tc = nav[i].logout
            ? (hov ? CS_ERR : (Color){255,120,170,255})
            : (isAct ? CA_GOLD : (hov ? CT_WHITE : CT_MUTED));
        DrawTxtVCL(nav[i].label, r, 14.f, tc, 14.f, false);

        if (hov) {
            SetMouseCursor(MOUSE_CURSOR_POINTING_HAND);
            if (IsMouseButtonReleased(MOUSE_LEFT_BUTTON)) {
                if (nav[i].logout) { gCurrentStaff = NULL; resetScreen(SCR_LOGIN); }
                else resetScreen(nav[i].scr);
            }
        }
    }
...
static void drawLogin(void) {
    ...
    /* Header card */
    DrawRectangleRounded((Rectangle){card.x,card.y,(float)cw,100},0.16f,12,(Color){225,235,255,255});

    /* Coffee icon */
    float ix=cx+cw*.5f, iy=cy+34.f;
    DrawCircle((int)ix,(int)iy,20,(Color){90,140,255,255});
    DrawCircle((int)ix,(int)iy,14,CA_GOLD);
    DrawCircle((int)ix,(int)iy, 9,(Color){90,140,255,255});
    DrawCircle((int)ix,(int)iy, 4,CA_GOLD);
    for (int i=0;i<3;i++){
    ...
    DrawTxtL(sub, cx+(cw-stv.x)*.5f, cy+100.f, 13.f, CT_MUTED);

    /* Form inputs */
    int fx=cx+48, fw=cw-96;
    DrawTxtL("Tên đăng nhập",(float)fx, cy+130.f, 13.f, CT_MUTED);
    InputField((Rectangle){(float)fx,(float)(cy+150),fw,46},0,"Nhập username...",15);

    DrawTxtL("Mật khẩu",(float)fx, cy+212.f, 13.f, CT_MUTED);
    gInpPass[1]=true;
...
static void drawTableMap(void) {
    ...
    struct { const char *lbl,*val; Color col; } kpis[3];
    char k1[16],k2[16],k3[32];
    sprintf(k1,"%d / %d",occ,MAX_TABLES);   kpis[0]=(typeof(kpis[0])){"Đang phục vụ",k1,CS_ERR};
    sprintf(k2,"%d / %d",MAX_TABLES-occ,MAX_TABLES); kpis[1]=(typeof(kpis[1])){"Bàn trống",k2,CS_OK};
    sprintf(k3,"%.0f VND",liveBill);           kpis[2]=(typeof(kpis[2])){"Tạm thu",k3,CA_GOLD};

    int kw=(aw-60)/3, kh=68;
    for (int i=0;i<3;i++){
        Rectangle kr={(float)(ax+16+i*(kw+14)),(float)(ay+12),(float)kw,(float)kh};
        DrawRectangleRounded(kr,0.12f,8,CB_PANEL);
        DrawRectangle((int)kr.x,(int)kr.y,3,kh,kpis[i].col);
        DrawTxtL(kpis[i].lbl, kr.x+14, kr.y+10, 12.f, CT_MUTED);
        DrawTextEx(gFontB, kpis[i].val, (Vector2){kr.x+14,kr.y+30}, 20.f, 1.f, kpis[i].col);
    }

    /* Lưới bàn 5×2 */
    int cols=5, rows=2, pad=14;
    int tw=(aw-pad*(cols+1))/cols, th2=148;
    int gx=ax+pad, gy=ay+96;
    ...
    for (int i=0;i<MAX_TABLES;i++){
        ...
        /* shadow */
        DrawRectangleRounded((Rectangle){r.x+3,r.y+4,(float)tw,(float)th2},
            0.12f,8,(Color){120,160,255,(unsigned char)(28*(0.3f+t*.7f))});

        Color bg={
            (unsigned char)lerp((float)CB_CARD.r,(float)CB_CARD_HOV.r,t),
            (unsigned char)lerp((float)CB_CARD.g,(float)CB_CARD_HOV.g,t),
            (unsigned char)lerp((float)CB_CARD.b,(float)CB_CARD_HOV.b,t),255};
        DrawRectangleRounded(r,0.12f,8,bg);

        /* top stripe */
        Color sc=occ2?CS_ERR:CS_OK;
        DrawRectangleRounded((Rectangle){r.x+1,r.y,(float)(tw-2),7},0.9f,4,sc);

        /* số bàn */
        char tn[16]; sprintf(tn,"BÀN %02d",gTables[i].id);
        ...
        if (occ2){
            ...
            DrawTextEx(gFontB,bs,(Vector2){fx+(tw-bv.x)*.5f,fy+66},16.f,1.f,CA_GOLD);
...
static void drawOrder(void) {
    ...
    int vi=0;
    for (int i=0;i<gMenuCount;i++){
        if (gInpLen[0]>0 && !matchSearch(gInp[0],gMenu[i].name)) continue;
        float fy = (float)(listStartY + vi*itemH) - gScrollY;
        vi++;
        if (fy+itemH<listStartY || fy>listStartY+visH) continue;

        Rectangle r={(float)(menuX+8),fy,(float)(menuW-16),(float)(itemH-4)};
        Vector2 mp=GetMousePosition();
        bool hov=CheckCollisionPointRec(mp,r);
        ...
        if (oos){
            ...
            if (Button(ab,"+ Thêm",CA_GOLD_DIM,CA_GOLD,slot)){
                if (!gTables[t].isOccupied){
                    ...
                    if (gCurrentStaff) strcpy(gTables[t].staffName,gCurrentStaff->name);
                }
                bool found2=false;
                ...
                char msg[64]; sprintf(msg,"Đã thêm: %s",gMenu[i].name);
                showToast(msg,CS_OK);
            }
        }
    }
    ...
    DrawRectangle(ordX,ay,ordW,48,CB_BG);
    ...
    int oStartY=ay+52, oItemH=52, oVisH=ah-52-112;
    ...
    for (int i=0;i<gTables[t].itemCount;i++){
        ...
        if (CheckCollisionPointRec(GetMousePosition(),xr)&&IsMouseButtonReleased(MOUSE_LEFT_BUTTON)){
            ...
            gTables[t].itemCount--;
            if(!gTables[t].itemCount){gTables[t].isOccupied=0;gTables[t].currentBill=0;}
        }
    }
    EndScissorMode();

    /* Tổng tiền */
    DrawRectangle(ordX,ay+ah-110,ordW,1,CB_BORDER);
    ...
    DrawTextEx(gFontB,tot,(Vector2){ordX+ordW-totv.x-10,ay+ah-108},22.f,1.f,CA_GOLD);

    /* Nút hành động */
    float bW=(ordW-20)*.5f;
    if (Button((Rectangle){ordX+6,ay+ah-74,bW,32},"Quay lại",CB_CARD,CB_CARD_HOV,4))
        resetScreen(SCR_TABLES);
    if (Button((Rectangle){ordX+14+bW,ay+ah-74,bW,32},"Thanh toán",CA_GOLD_DIM,CA_GOLD,5)){
        if (gTables[t].itemCount>0) { gScreen=SCR_INVOICE; }
        else showToast("Bàn chưa có món nào!",CS_WARN);
    }
    if (Button((Rectangle){ordX+6,ay+ah-36,ordW-12,28},"Làm mới đơn",CB_CARD,(Color){45,26,12,255},6)){
...
static void drawMenuManage(void) {
    ...
    if (gSubScr==0) {
        ...
        /* Rows */
        int rowH=50, startY=hY+36, visH=ah-56-36;
        ...
        for (int i=0;i<gMenuCount;i++){
            if (gInpLen[0]>0 && !matchSearch(gInp[0],gMenu[i].name)) continue;
            float fy=(float)(startY+vi*rowH)-gScrollY; vi++;
            if (fy+rowH<startY||fy>startY+visH) continue;
            ...
            if (SmallBtn((Rectangle){ax+664,by,52,26},"Sửa",CB_CARD,CB_CARD_HOV,20+i*3)){
                ...
                strncpy(gInp[0],gMenu[i].name,1019); gInpLen[0]=(int)strlen(gInp[0]);
                char pc[20]; sprintf(pc,"%.0f",gMenu[i].price);
                strncpy(gInp[1],pc,1019); gInpLen[1]=(int)strlen(gInp[1]);
            }
            const char *tl=gMenu[i].inStock?"Hết":"Còn";
            if (SmallBtn((Rectangle){ax+720,by,48,26},tl,CB_CARD,CB_CARD_HOV,21+i*3)){
                gMenu[i].inStock=!gMenu[i].inStock; saveMenu();
                showToast("Đã cập nhật trạng thái!",CS_INFO);
            }
            if (SmallBtn((Rectangle){ax+772,by,48,26},"Xóa",(Color){255,120,160,255},CS_ERR,22+i*3)){
        ...
        DrawTextEx(gFontB,ft,(Vector2){fx+(fw-ftv.x)*.5f,fy+14},16.f,1.f,CA_GOLD);
        ...
        bool sv=Button((Rectangle){px,fy+216,(float)(pw/2-8),44},"💾  Lưu",CA_GOLD_DIM,CA_GOLD,1);
        bool cl=Button((Rectangle){px+pw/2+8,fy+216,(float)(pw/2-8),44},"✕  Hủy",CB_CARD,CB_CARD_HOV,2);
        ...
        if (sv){
            if (!gInpLen[0]||!gInpLen[1]) showToast("Vui lòng điền đầy đủ thông tin!",CS_WARN);
                ...
                if (gSubScr==1){
                    if (gMenuCount>=MAX_MENU) showToast("Danh sách đã đầy!",CS_ERR);
                    else {
                        ...
                        gMenuCount++; saveMenu();
                        showToast("Đã thêm món mới!",CS_OK); gSubScr=0;
                    }
                } else {
                    for(int j=0;j<gMenuCount;j++) if(gMenu[j].id==gEditId){
                        strncpy(gMenu[j].name,gInp[0],STR_LEN-1);
                        gMenu[j].price=price; saveMenu();
                        showToast("Đã cập nhật món!",CS_OK); gSubScr=0; break;
                    }
                }
            }
        }
...
static void drawStaffManage(void) {
    ...
    if (gSubScr==0){
        ...
        int vi=0;
        for(int i=0;i<gStaffCount;i++){
            if(gInpLen[0]>0&&!matchSearch(gInp[0],gStaff[i].name)) continue;
            float fy=(float)(startY+vi*rowH)-gScrollY; vi++;
            if(fy+rowH<startY||fy>startY+visH) continue;
            Color rbg=(vi%2==0)?CB_ROW_ALT:CB_BG;
            DrawRectangle(ax,(int)fy,aw,rowH,rbg);
            ...
            if(gStaff[i].id!=100){
                if(SmallBtn((Rectangle){ax+756,by,52,26},"Sửa",CB_CARD,CB_CARD_HOV,20+i*2)){
                    gSubScr=2; gEditId=gStaff[i].id;
                    strncpy(gInp[0],gStaff[i].username,1019); gInpLen[0]=(int)strlen(gInp[0]);
                    strncpy(gInp[2],gStaff[i].name,1019);    gInpLen[2]=(int)strlen(gInp[2]);
                    strncpy(gInp[3],gStaff[i].position,1019);gInpLen[3]=(int)strlen(gInp[3]);
                    char sh2[24]; sprintf(sh2,"%.0f",gStaff[i].salaryPerHour);
                    strncpy(gInp[4],sh2,1019); gInpLen[4]=(int)strlen(gInp[4]);
                }
                if(SmallBtn((Rectangle){ax+812,by,52,26},"Xóa",(Color){255,120,160,255},CS_ERR,21+i*2)){
        ...
        if (sv){
            ...
            if(isAdd) ok=ok&&gInpLen[1];
            if(!ok) showToast("Vui lòng điền đầy đủ thông tin!",CS_WARN);
            else {
                if(isAdd){
                    if(gStaffCount>=MAX_STAFF) showToast("Danh sách đã đầy!",CS_ERR);
                    else {
                        ...
                        gStaffCount++; saveStaff();
                        showToast("Đã thêm nhân viên thành công!",CS_OK); gSubScr=0;
                    }
                } else {
                    for(int j=0;j<gStaffCount;j++) if(gStaff[j].id==gEditId){
                        strncpy(gStaff[j].name,    gInp[2],STR_LEN-1);
                        strncpy(gStaff[j].position,gInp[3],SHORT_STR-1);
                        gStaff[j].salaryPerHour=atof(gInp[4]);
                        saveStaff(); showToast("Đã cập nhật nhân viên!",CS_OK);
                        gSubScr=0; break;
                    }
                }
            }
...
static void drawStats(void) {
    ...
    /* KPI cards */
    struct{const char *lbl,*sub;float val;const char *unit;Color col;}kpis[]={
    ...
    for(int i=0;i<3;i++){
        ...
        DrawTxtL(kpis[i].lbl,kr.x+16,kr.y+12,13.f,CT_MUTED);
        char vs[48]; sprintf(vs,"%.0f",kpis[i].val);
        char full[64]; sprintf(full,"%s%s",vs,kpis[i].unit);
        DrawTextEx(gFontB,full,(Vector2){kr.x+16,kr.y+36},22.f,1.f,kpis[i].col);
        DrawTxtL(kpis[i].sub,kr.x+16,kr.y+70,11.f,CT_DIM);
    }
    ...
    gridY+=34;
    int tcols=5, tw2=(aw-28)/tcols, th3=66;
    for(int i=0;i<MAX_TABLES;i++){
        int col=i%tcols, row=i/tcols;
        Rectangle tr={(float)(ax+14+col*tw2),(float)(gridY+row*(th3+4)),(float)(tw2-4),(float)th3};
        bool occ2=gTables[i].isOccupied;
        Color tbg=occ2?(Color){255,225,235,255}:(Color){225,245,235,255};
        DrawRectangleRounded(tr,0.12f,6,tbg);
        Color sc2=occ2?CS_ERR:CS_OK;
        DrawRectangleRoundedLines(tr,0.12f,6,sc2);
        char tn[16]; sprintf(tn,"Bàn %02d",gTables[i].id);
        Vector2 tnv=Measure(tn,12);
        DrawTxtL(tn,tr.x+(tr.width-tnv.x)*.5f,tr.y+6,12.f,CT_WHITE);
        if(occ2){
            char bs[24]; sprintf(bs,"%.0f đ",gTables[i].currentBill);
            Vector2 bv=MeasureB(bs,14);
            DrawTextEx(gFontB,bs,(Vector2){tr.x+(tr.width-bv.x)*.5f,tr.y+28},14.f,1.f,CA_GOLD);
        } else {
            Vector2 fv=Measure("Trống",12);
            DrawTxtL("Trống",tr.x+(tr.width-fv.x)*.5f,tr.y+28,12.f,sc2);
        }
    }
    ...
    if (fr){
        ...
        while(fgets(line,256,fr)&&li<7){
            ...
            DrawRectangle(ax+14,histY+li*28,aw-28,27,lbg);
            DrawTxtL(line,ax+24,histY+li*28+8,12.f,CT_WHITE);
            li++;
        }
        fclose(fr);
    } else {
...
static void drawDialog(void) {
    ...
    bool yes=Button((Rectangle){dx+36,dy+126,(float)(dw/2-50),46},"✓  Có, xóa ngay",yesBg,yesHov,80);
    bool no =Button((Rectangle){dx+dw/2+14,dy+126,(float)(dw/2-50),46},"✕  Không",noBg,noHov,81);
    ...
    if (yes) {
        ...
        if (act>=100&&act<200){  /* xóa menu */
            ...
            if(idx>=0&&idx<gMenuCount){
                for(int j=idx;j<gMenuCount-1;j++) gMenu[j]=gMenu[j+1];
            ...
            if(idx>=0&&idx<gStaffCount&&gStaff[idx].id!=100){
                for(int j=idx;j<gStaffCount-1;j++) gStaff[j]=gStaff[j+1];
...
int main(void) {
    ...
    gFont  = tryLoadFont(regularPaths, 22);
    gFontB = tryLoadFont(boldPaths,    22);
    if (gFontB.baseSize == 0) gFontB = gFont;

    /* Khởi tạo dữ liệu */
    loadStaff(); loadMenu();
    for (int i=0;i<MAX_TABLES;i++){
        gTables[i].id=i+1; gTables[i].isOccupied=0;
        gTables[i].itemCount=0; gTables[i].currentBill=0;
    }
    ...
    while (!WindowShouldClose()) {
        ...
        switch (gScreen) {
            ...
            case SCR_TABLES:  drawTableMap();     break;
            case SCR_ORDER:   drawOrder();        break;
            case SCR_INVOICE: drawInvoice();      break;
            case SCR_MENU:    drawMenuManage();   break;
            case SCR_STAFF:   drawStaffManage();  break;
            case SCR_STATS:   drawStats();        break;
            case SCR_CHART:   drawChart();        break;
        }
    ...
    if (gFontB.baseSize != gFont.baseSize) UnloadFont(gFontB);
    UnloadFont(gFont);
    CloseWindow();
    return 0;
}
